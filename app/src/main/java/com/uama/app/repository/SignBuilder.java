...
public class SignBuilder {
    ...
    public static Map<String, String> getBaseMap(Context context, String time) {
      ...
      /*  if (!TextUtils.isEmpty(PreferenceUtils.getToken())) {
            params.put("token", PreferenceUtils.getToken());//
        }
        if (!TextUtils.isEmpty(PreferenceUtils.getCommunityId())) {
            params.put(PreferenceUtils.COMMUNITY_ID, PreferenceUtils.getCommunityId());
        }
        if (!TextUtils.isEmpty(PreferenceUtils.getRoomId())) {
            params.put(PreferenceUtils.ROOM_ID, PreferenceUtils.getRoomId());
        }
        //新增head字段 defOrgId 组织id
        if(!TextUtils.isEmpty(DataConstants.getOrgId())) {
            params.put(PreferenceUtils.DEF_ORG_ID, DataConstants.getOrgId());
        }
        params.put("companyCode", Constants.companyCode);//必传
        params.put("mobileType", Constants.REQUEST_COMM);//必传
        params.put("mobileVersion", DeviceUtils.getSystemCode());
        params.put("appVersion", DeviceUtils.getVersionName(context));
        params.put("mobileNo", DeviceUtils.getUUID());
        params.put("mobileName", DeviceUtils.getDeviceName());
        params.put("time", time);
        params.put("version", Constants.VERSION);
        params.put("defLatitude", StringUtils.newString(DataConstants.getLocationInfo().latitude));
        params.put("defLongitude", StringUtils.newString(DataConstants.getLocationInfo().longitude));
        params.put("defAreaCode", StringUtils.newString(DataConstants.getLocationInfo().areaCode));*/
        return params;
    }
}
